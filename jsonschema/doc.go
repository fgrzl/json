...
// exclusiveMaximum, minItems, maxItems, uniqueItems, $ref (same-document
// #/$defs/X and #/components/schemas/X), allOf, anyOf, oneOf, not.
//
// Keywords
//
...
// []byte, json.RawMessage, sql.Null*) are process-wide global state. Registration
// is not safe for concurrent use. Tests that need a clean slate should call
// ClearRegistry to restore the default built-in set and remove custom registrations.
package jsonschema
